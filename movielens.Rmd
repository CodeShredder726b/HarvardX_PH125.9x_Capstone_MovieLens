...
First we transform the UNIX timestamps to date/time and for ease of use also the year.
```{r echo=TRUE}
edx_1 <- edx %>% 
...
```{r echo=TRUE}
#edx[,list(`movies` = uniqueN(movieId), `users` = uniqueN(userId), `genres` = uniqueN(genres))]
...
There are `r length(unique(edx$movieId))` rated movies by `r length(unique(edx$userId))`. There are `r ....` genres, mostly a movie is a mix of multiple genres.
